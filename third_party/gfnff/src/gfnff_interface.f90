...
!========================================================================================!
  ...
  &                 printlevel,printunit,version,iostat,ichrg,lattice,npbc)
    ...
    integer :: ich,io,mylevel,myunit,ib
    logical :: graph_valid
    logical :: ex
    logical :: restart
...
!> BEAUTIZE: validate and install the exact host graph before any topology
!> setup. No silent fallback on invalid input. Preserve the old path when absent.
    if (allocated(dat%userinput)) then
      if (allocated(dat%userinput%bond_orders)) then
        graph_valid = nat > 0
        if (present(npbc)) graph_valid = graph_valid .and. npbc == 0
        graph_valid = graph_valid .and. all(shape(dat%userinput%bond_orders) == [nat,nat])
        if (graph_valid) then
          associate (bo => dat%userinput%bond_orders)
            graph_valid = all(ieee_is_finite(bo))
            if (graph_valid) then
            graph_valid = all(bo >= 0.0_wp)
            graph_valid = graph_valid .and. maxval(abs(bo-transpose(bo))) < 1.0e-12_wp
            do ib = 1,nat
              graph_valid = graph_valid .and. bo(ib,ib) == 0.0_wp
              graph_valid = graph_valid .and. count(bo(:,ib)>0.0_wp) <= dat%neigh%numnb-2
            end do
            if (allocated(dat%userinput%fraglist)) then
              if (size(dat%userinput%fraglist) /= nat) then
                ...
                do ib = 1,nat
                  if (any(bo(:,ib)>0.0_wp .and. &
                    & dat%userinput%fraglist /= dat%userinput%fraglist(ib))) graph_valid = .false.
                end do
              end if
            end if
            end if ! finite bond-order array
          end associate
        end if
        if (.not.graph_valid) then
          if (mylevel >= 1) write(myunit,'(a)') '**ERROR** invalid explicit GFN-FF molecular graph'
          if (present(iostat)) iostat = -20
          return
        end if
        dat%neigh%user_bond_orders = dat%userinput%bond_orders
        dat%restart = .false.
      end if
    end if

...
! done with main groups
    ...
    ! BEAUTIZE: reduced lists may determine hybridisation, but must NEVER
    ! remove a user-specified edge from the final bonded graph (including eta bonds).
    if (allocated(neigh%user_bond_orders)) neigh%nb = neigh%nbf
    deallocate (nbdum)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine gfnff_hbset(n,at,xyz,topo,neigh,nlist,hbthr1,hbthr2)
    ...
    ! BEAUTIZE portability fix: buffers and flushes are required in serial builds too.

...
module gfnff_neighbor
  ...
  type :: TNeigh
    ...
    !> BEAUTIZE: immutable exact molecular graph, populated before setup.
    real(wp),allocatable :: user_bond_orders(:,:)
  contains
    ! initialize neigh
    procedure :: init_n
    ...
    do iTr = 1,self%numctr
      do i = 1,n
        do j = 1,n
          ...
          k = lin(j,i)
          ! BEAUTIZE: an external graph replaces ALL distance bond detection.
          if (allocated(self%user_bond_orders)) then
            if (iTr /= 1) cycle
            if (self%user_bond_orders(j,i) <= 0.0_wp) cycle
          else
